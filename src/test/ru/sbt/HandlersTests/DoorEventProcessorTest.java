...
public class DoorEventProcessorTest extends TestCase {
    ...
    public void testOpenSomeDoors() {
        ...
        for (Room room : rooms) {
            for (Door door : room.getDoors()) {
                if (door.getId().equals(closedDoors)) assertTrue(door.isOpen());
                else assertFalse(door.isOpen());
            }
        }
    }
    ...
    public void testCloseSomeDoors() {
        ...
        for (Room room : rooms) {
            for (Door door : room.getDoors()) {
                if (door.getId().equals(closedDoors)) assertFalse(door.isOpen());
                else assertTrue(door.isOpen());
            }
        }
    }
